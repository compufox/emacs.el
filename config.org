...
#+begin_src emacs-lisp  
  ...
  (cl-defun asdf-generate-makefile (&optional dir)
    "generates a makefile for a common lisp project"
    (interactive)
    (let* ((cwd (or dir (file-name-directory buffer-file-truename)))
           (asd-file (car (focks/asdf-project-dir-p cwd))))
      (unless asd-file
  ...
  (cl-defun asdf-add-build-instructions (&optional dir)
  ...
  (defun asdf-add-build-and-makefile ()
    ...
    (cl-flet ((ivy-find-dir (prompt)
      ...
      (let ((asdf-dir (when current-prefix-arg (ivy-find-dir "ASDF Project Dir: "))))
        (when (and (y-or-n-p "Generate a Makefile and add build instructions for lisp system?")
                   (and current-prefix-arg asdf-dir))
