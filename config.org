...
#+begin_src emacs-lisp  
  ...
  (cl-defun asdf-generate-makefile ()
    "generates a makefile for a common lisp project"
    (interactive)
    (let* ((cwd (file-name-directory buffer-file-truename))
  ...
  (cl-defun asdf-add-build-instructions ()
  ...
  (defun asdf-add-build-and-makefile ()
    ...
    (interactive)
    (when (y-or-n-p "Generate a Makefile and add build instructions for the current lisp system?")
      (asdf-generate-makefile)
      (asdf-add-build-instructions)))
#+end_src
