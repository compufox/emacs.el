...
#+begin_src emacs-lisp  
  ...
  (defun compute-bra-size (band-size bust-size)
    "interactive wrapper around bra-size-math function"
    (interactive "sBand Size: \nsBust Size: ")
    (require 'calc-units) ;; this is needed because #'math-convert-units is in calc-unit's package
    (message (bra-size-math band-size bust-size)))

  (defun bra-size-math (band-expr bust-expr &optional region)
    "since this function uses emacs' internal unit conversion, BAND-EXPR and BUST-EXPR need to be passed with units ('9 in', '90 cm', etc). 
  REGION is a symbol representing which region's sizing charts to use. possible values are 'US, 'EU, 'UK. (defaults to 'US)

  this is taken from https://pulusound.fi/blog/emacs-bra-size-calculator"
...
Go ahead and load the packaging subsystem so that we can have access to MELPA packages early
...
#+begin_src emacs-lisp
  ...
  (add-hook 'prog-mode-hook
            #'(lambda ()
                (when (member major-mode '(emacs-lisp-mode lisp-mode lisp-interaction-mode))
...
#+begin_src emacs-lisp
  ...
  returns either 'dark or 'light"
           (let ((theme (shell-command-to-string (concat "osascript " focks/*config-root* "extras/CheckSystemTheme.scpt"))))
...
#+begin_src emacs-lisp
  ...
  (use-package dap-mode
    ...
    (setq dap-auto-configure-features '(sessions locals controls tooltip)
          dap-lldb-debug-program (list (concat (getenv "HOME") "/.swiftly/bin/lldb-dap")))
    (require 'dap-lldb))
  ...
  (use-package lsp-sourcekit
    ...
    (setq lsp-sourcekit-executable
          (string-trim (shell-command-to-string
                        (focks/os-cond
                         (windows "where.exe sourcekit-lsp")
                         (t "which sourcekit-lsp"))))))
  ...
  (use-package mindstream
    :ensure t
    :custom (mindstream--template-path "~/.config/mindstream/templates")
  ...
  (use-package parinfer-rust-mode
    ...
    (parinfer-rust-library
     (focks/os-cond
      (windows "~/.emacs.d/parinfer-rust/parinfer_rust.dll")
  ...
  ;; this is disabled in daemon-mode until bugfixes get merged
  (use-package prism
    :ensure t
    :unless (daemonp)
    :hook (prog-mode . prism-mode))

  ;; this bandaid seems to work in the meantime
  (when (daemonp)
  ...
  (use-package emojify
    ...
    (when-on-windows
     ...
     (set-fontset-font t 'symbol (font-spec :family "Segoe UI Symbols"))))
  ...
  (use-package gdscript-mode
    :ensure straight
    :after lsp-mode
    :hook (gdscript-mode . lsp-mode)
    :straight (gdscript-mode :type git
                             :host github
  ...
  (use-package ligature
    ...
    ;; these ligatures are configured for Cartograph CF 2.9.4
