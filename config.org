...
#+begin_src emacs-lisp
  ...
  (defun focks/get-system-arch ()
    ...
    (focks/os-cond
     ((unix macos) (car (split-string (shell-command-to-string "uname -m") nil t)))
     (t "x86_64")))
...
#+begin_src emacs-lisp  
  ...
  INCLUDES is a space seperated list of headers to include"
    ...
    (let ((path (apply #'file-name-concat 
  ...
  (defun focks/load-emacs-theme (&optional mode)
    ...
    ensures disabling all prior loaded themes before changing"
      (cl-flet ((load-themes (x)
        ...
        (if (eq 'dark (or mode focks/enable-dark-theme))
...
#+begin_src emacs-lisp
  (when-on-macos
   ...
   (if (boundp 'ns-system-appearance)
       (progn
         (setq focks/enable-dark-theme ns-system-appearance)
  ...
  returns either 'dark or 'light"
       ...
       (defun focks/match-theme-to-system ()
         ...
         (unless (equal focks/*current-theme* (focks/macos-theme))
           (setq focks/*current-theme* (focks/macos-theme))
...
#+begin_src emacs-lisp
  (set-face-attribute 'default nil
                      ...
                      :width 'normal
                      :weight 'normal
                      :family (font-get (or (focks/font-available-p "Cartograph CF")
                                            (focks/font-available-p "Chalkboard")
                                            (focks/font-available-p "MS Comic Sans"))
                                        :name))
  (setq font-lock-comment-face '(:slant italic :family "Cartograph CF"))
