...
This part of the configuration exists solely to document the =bootstrap.el= file that handles the loading and parsing of this config.
It does not get tangled or loaded during the standard start process.
...
#+begin_src emacs-lisp 
  ...
  ;; checks and ensures that the init directory is created
  (unless (file-directory-p focks/*init-dir*)
    (make-directory focks/*init-dir*))
...
Using the standard org-babel tangle function to tangle and load a file (=org-babel-load-file=) created clutter in my git repo that I did not like (among other things), so I wrote custom logic to manually tangle and load this config.
